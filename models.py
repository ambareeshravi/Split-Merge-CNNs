...
def get_inception(num_classes = 2):
	'''
	Creates an instance of Incpetion V3 model
	'''
	return nn.Sequential(
		models.inception_v3(pretrained=False, num_classes = num_classes, aux_logits = False),
		nn.Softmax()
	)
...
def get_mobilenet(num_classes=2):
	...
	'''
	return nn.Sequential(
		models.MobileNetV2(num_classes=num_classes),
		nn.Softmax()
	)


class NormalCouterpart(nn.Module):
	'''
	Implements the Normal Counter part of the Split-Merge CNN classification architecture
	'''
	def __init__(self, output_nodes = 1, ngpu = 1):
		'''
		...
		Returns:
			-
		'''
        super(NormalCouterpart, self).__init__()

        self.filters_count = [64, 64, 128, 256, 128, 64]
        self.reduction_count = [64,32,16]
        
        self.ngpu = ngpu
        final_activation = nn.Sigmoid()
		if output_nodes > 1: final_activation = nn.Softmax()
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Dropout(0.25),
            nn.Linear(self.reduction_count[2], self.reduction_count[3]),
            nn.Sigmoid()
        )
...
class SplitMerge(nn.Module):
	...
	def __init__(self, output_nodes = 1, ngpu = 1):
		...
		self.classifier = nn.Sequential(
			...
			nn.Linear(self.reduction_count[1], self.reduction_count[2]),
			nn.Dropout(0.25),
			nn.Linear(self.reduction_count[2], output_nodes),
			final_activation
		)
