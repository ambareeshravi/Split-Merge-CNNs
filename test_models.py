...
class Tester:
	...
	def get_metrics(self, y_act, y_pred):
		...
		"ROC AUC Score": roc_auc_score(y_act, y_pred),
		"AVERAGE PRECISION SCORE": average_precision_score(y_act, y_pred),
		"MEAN AVERAGE PRECISON": " ".join(["| "+str(k)+"- "+str(ml_metrics.mapk(y_act.reshape(-1,1).tolist(), y_pred.reshape(-1,1).tolist(), k))[:9] for k in [5, 10, 50]]),
		"CLASSIFICATION REPORT": classification_report(y_act, y_pred),
		"CONFUSION MATRIX": confusion_matrix(y_act, y_pred)
		}
	...
	def test(self, model_path, debug = False):
		...
		results = self.get_metrics(y_act, y_pred)
		print("-"*10, " RESULTS", "-"*10)
		for k, v in results.items():
			print(k, v)
		print("="*30)
		with open(os.path.splitext(model_path)[0] + "_Results.pkl", "wb") as f:
			pkl.dump(results, f)
...
if __name__ == "__main__":
	ts = Tester()
	ts.test(model_type = "SplitMerge_XRay_V1_FINAL.pth", debug = True)